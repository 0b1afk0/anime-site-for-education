...
<h3 style="color:#c95987;">Добавить просмотренное аниме</h3>

<form method="POST" action="{{ route('admin.watched.store') }}">
    @csrf
    ...
    <div class="mb-3">
        ...
        <select name="anime_id" class="form-control">
            @foreach($animes as $item)
                <option value="{{ $item->id }}" {{ old('anime_id') == $item->id ? 'selected' : '' }}>
                    {{ $item->title }}
                </option>
            @endforeach
        </select>
